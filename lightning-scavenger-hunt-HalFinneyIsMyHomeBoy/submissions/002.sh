...
wd=$(pwd)
lncli=$(lncli -n=signet --rpcserver=35.209.148.157:10009 --tlscertpath="$wd"/test/lnd0-tls.cert --macaroonpath="$wd"/test/lnd0-readonly.macaroon getchaninfo $channel_id)
echo $lncli | jq -r '.node2_policy.fee_base_msat'